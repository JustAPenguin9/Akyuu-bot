...
async fn main() {
	...
	let squiroll_messages = Arc::new(RwLock::new(squiroll_messages));

	// communicate with squiroll lobby to get the online players
	tokio::spawn(update_lobby_data(lobby_data.clone()));
	// update discord messages with updated squiroll lobby players
	tokio::spawn(update_lobby_messages(
		http.clone(),
		lobby_data.clone(),
		squiroll_messages.clone(),
	));
	...
	let framework = poise::Framework::builder()
				...
				Box::pin(async move {
					match event {
						Ready { data_about_bot } => {
							...
							{
								// used in update_lobby_messages
								let mut http = data.http.lock().await;
		...
		.setup(move |_ctx, _ready, _framework| {
			Box::pin(async move { Ok(Data { pool, http, lobby_data, squiroll_messages }) })
		})
		.build();
